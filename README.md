...
Язык не $LL(k)$, воспользуемся методом подмены:
...
применим метод подмены во втором слове для префикса $с^j$, получим слово 
$$a^{n+k}cac^{n+k-j-1}a^i$$
которое не принадлежит рассматриваемому языку.
